...
const COOKIE_OPTS: Cookies.CookieAttributes = { path: '/', expires: 30, sameSite: 'Lax' };
...
export const getAccessToken = (): string | null => Cookies.get(TOKEN_COOKIE) || null;
...
export const setTokens = ({ access_token, refresh_token }: TokenPair): void => {
  ...
  if (refresh_token) Cookies.set(REFRESH_TOKEN_COOKIE, refresh_token, COOKIE_OPTS);
};
...
export const decodeAccessToken = (token?: string | null): AccessTokenPayload | null => {
...
export const setSignupUser = ({ id, email, first_name, last_name }: SignupUser): void => {
  Cookies.set(
    SIGNUP_USER_COOKIE,
    JSON.stringify({ id, email, first_name: first_name || '', last_name: last_name || '' }),
...
export const getResetEmail = (): string | null => Cookies.get(RESET_EMAIL_COOKIE) || null;

export const setResetEmail = (email: string): void => {
  Cookies.set(RESET_EMAIL_COOKIE, email, { path: '/', expires: 1, sameSite: 'Lax' });
};
...
export const getResetToken = (): string | null => Cookies.get(RESET_TOKEN_COOKIE) || null;

export const setResetToken = (token: string): void => {
  Cookies.set(RESET_TOKEN_COOKIE, token, { path: '/', expires: 1, sameSite: 'Lax' });
};
