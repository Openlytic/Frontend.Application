...
const CreateAccountPage = () => {
  ...
  const handleSubmitOrg = async () => {
    ...
    try {
      ...
      const locationPreset = LOCATION_PRESETS.find(
        (p) => p.country_code === org.country_code,
      );
  ...
  const checkSubDomain = async (_: unknown, value: string) => {
    ...
    if (slug !== value) {
      return Promise.reject(
        new Error("Use lowercase letters, numbers and dashes only"),
      );
    }
    try {
      ...
      {step === 1 && (
        ...
        >
          ...
          <Form.Item
            name="sub_domain"
            rules={[
              { required: true, message: "Choose a sub-domain" },
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Form.Item name="country_code" label="Country">
              <Select
                options={LOCATION_PRESETS.map((p) => ({
